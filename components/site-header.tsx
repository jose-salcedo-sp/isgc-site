...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-tinto text-white shadow-sm">
      <div className="mx-auto flex max-w-[1200px] items-center justify-between gap-5 px-5 py-3 lg:px-6">
        <Link href="/" className="flex shrink-0 items-center gap-3" aria-label="ISGC, inicio">
          <Image src="/isgc-logo-embedded.png" alt="ISGC" width={39} height={45} className="h-11 w-auto object-contain" priority />
          <span className="pl-1 leading-tight">
            <span className="block text-base font-bold tracking-[0.12em]">ISGC</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/alumnos" className="rounded px-2 py-2 text-sm font-semibold text-white lg:hidden">
            Alumnos
          </Link>
          <button
            type="button"
      ...
      {open && (
        <nav id="mobile-navigation" className="px-5 py-3 lg:hidden" aria-label="Navegación móvil">
          <div className="mx-auto grid max-w-[1200px] gap-1">
            ...
            <Link href="/aspirantes" onClick={() => setOpen(false)} className="mt-2 rounded-full bg-white/10 px-3 py-3 font-bold text-white">
              Admisiones <span aria-hidden="true">↗</span>
            </Link>
          </div>
        </nav>
